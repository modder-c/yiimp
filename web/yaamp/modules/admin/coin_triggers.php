...
{
	if ($rule->enabled)
		$operations = '<a title="Disable this rule" href="/admin/cointriggerEnable?id='.$rule->id.'&en=0">disable</a>';
	else
		$operations = '<a title="Enable this rule" href="/admin/cointriggerEnable?id='.$rule->id.'&en=1">enable</a>';
	$operations .= '&nbsp;<a class="red" title="Remove this market" href="/admin/cointriggerDel?id='.$rule->id.'">delete</a>';

	if ($rule->lasttriggered && $rule->lasttriggered == $rule->lastchecked) {
		$status = '<span class="green">Triggered</span>';
		$operations = '<a title="Reset trigger" href="/admin/cointriggerReset?id='.$rule->id.'">reset</a>'.'&nbsp'.$operations;
	} else {
...
echo <<<end
<div class="form">
<form action="/admin/cointriggerAdd?id={$coin->id}" method="post" style="padding: 0px;">
