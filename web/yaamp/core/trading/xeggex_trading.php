<?php
function doXeggexCancelOrder($OrderID = false) {
	if(!$OrderID) return;

	$params = json_encode([ 'id' => $OrderID ]);

	$res = xeggex_api_user('cancelorder', $params, 'POST' ,'array');

	if(is_array($res)) {
		$db_order = getdbosql('db_orders', "market=:market AND uuid=:uuid", array(
			':market'=>'xeggex', ':uuid'=>$OrderID
		));
		if($db_order) $db_order->delete();
	}
}

function doXeggexTrading($quick=false) {

	$exchange = 'xeggex';
	$updatebalances = true;
	if (exchange_get($exchange, 'disabled')) return;

	$balances = xeggex_api_user('balances','','GET','array');
	//debuglog("xeggex ".var_export($balances,true));
	...
	$marketprices = xeggex_api_query('tickers','','array');
	...
	foreach ($balances as $balance) {
		...
		$market = getdbosql('db_markets', "coinid=:coinid AND name='xeggex'", array(':coinid'=>$coin->id));
		if(!$market) continue;
		...
		if ($heldForTrades > 0) {
			...
			$orders = xeggex_api_user('getorders', $params, 'GET', 'array' );
		}

		// debuglog("xeggex ".var_export($orders,true));
		...
		if(is_array($orders) && !empty($orders)) {
			foreach($orders as $order) {
				...
				if($sellprice > $ask*$cancel_ask_pct || $flushall) {
					debuglog("xeggex: cancel order ".$symbol." at $sellprice, ask price is now $ask");
					doXeggexCancelOrder($order['id']);
				}
				// store existing orders
				else
				{
					$db_order = getdbosql('db_orders', "market=:market AND uuid=:uuid", array(
							':market'=>'xeggex', ':uuid'=>$order['id']
					));
					if($db_order) continue;
	
					// debuglog("xeggex: store order of {$order->Amount} {$symbol} at $sellprice BTC");
					$db_order = new db_orders;
					$db_order->market = 'xeggex';
		...
		$list = getdbolist('db_orders', "coinid={$coin->id} AND market='xeggex'");
		foreach($list as $db_order)
		{
			...
			if(!$found) {
				// debuglog("xeggex: delete db order {$db_order->amount} {$coin->symbol} at {$db_order->price} BTC");
				$db_order->delete();
			}
		}
		...
		$market_configuration = xeggex_api_query($query_parameters, '', 'array');
		...
		$data = xeggex_api_query('orderbook', $orderparameters, 'array');

		//debuglog("xeggex ".var_export($data,true));
		...
		if($coin->sellonbid) {
			for($i = 0; ($i < 5) && ($amount >= 0); $i++) {
				...
				debuglog("xeggex: selling on bid $sellamount $symbol at $sellprice");
				$orderparameters = [ 'symbol' => strtoupper($balance['asset']).'_btc' ,
								 'price' => number_format($sellprice,10) ,
								 'side' => 'sell' ,
								 'quantity' => "$amount" ];
				$res = xeggex_api_user('createorder', json_encode($orderparameters) , 'POST', 'array');
	
				$res = false;
				if(!is_array($res)) {
					debuglog("xeggex SubmitTrade err: ".json_encode($res));
					continue;
				}
		...
		if($coin->sellonbid)
			...
			debuglog("xeggex: selling $amount $symbol at $sellprice");
